...
# 1장 마크 다운에 대하여
## 1.1. 마크 다운이란?
마크 다운(Mark-down)은 일반 텍스트 기반의 경량 마크업 언어이다. 2004년 존 그루버에 의해 만들어졌으며, 쉽게 읽고 쓸 수 있으며 HTML로 변환이 가능하다.

특수기호와 문자를 이용한 매우 간단한 구조의 문법을 사용하여 웹에서도 보다 빠르게 컨텐츠를 작성할 수 있고 직관적으로 인식할 수 있다.   
마크업 언어란 태그 등을 이용하여 문서나 데이터의 구조 등을 명기하는 언어의 한 가지이다. 텍스트만으로 서식이 있는 문서들을 작성할 때 자주 사용되며, 다른 마크업 언어들에 비해 문법이 쉽고 간단한 것이 특징이다.   
...
### 1.2.1. 장점
    ...
    3. 마크 다운으로 작성한 문서를 다양한 형태(포맷)로 손쉽게 변환이 가능하다.
...
## 2.2. 블록 인용구 (BlockQuote)
> 남의 말이나 글에서 직접 또는 간접으로 따온 문장.

> _(네이버 국어 사전)_

이메일(e-mail)에서 사용하는 ```>``` 블록 인용문자를 이용한다.
...
### ● 리스트 안에 리스트 생성
리스트 안에 리스트를 하려면 tab과 함께 숫자 1부터 나열하여 적용한다.
```
1. 리스트 1번
    1. 리스트 1-1번
2. 리스트 2번
...
문장 마지막에 3칸 이상의 띄어쓰기(후행 공백)를 포함하면 줄이 바뀐다.
```Enter```를 2회 입력 시 적용되기도 한다. (1회 입력 시 단락 변경이 되지 않는다.)
편집기 상에서 공백이 표기되지 않기 때문에 사용에 어려움이 있을 수 있다. 일부 outliner의 경우 bullet으로 구분하거나, HTML을 지원하는 편집기의 경우 ```<br>``` 태그를 사용하는 것이 좋다.
